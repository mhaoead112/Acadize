    ...
    <div dir={dir} className="flex h-screen w-full bg-slate-50 dark:bg-[#0a192f] overflow-hidden font-sans transition-colors duration-300">
      ...
      >
        ...
        <div className={`p-6 pb-2`}>
                ...
                <AnimatePresence>
                  {!isSidebarCollapsed && (
                    ...
                    >
                      ...
                      <p className="text-xs text-slate-600 dark:text-slate-400 truncate">{t('parentPortal')}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </>
          ...
          {/* User Profile Card */}
          <Link to="/settings">
            <div className={`flex gap-3 mb-6 bg-slate-50 dark:bg-[#112240] rounded-xl items-center border border-slate-100 dark:border-white/10 hover:bg-slate-100 dark:hover:bg-white/5 transition-colors cursor-pointer group px-3 py-3 ${isSidebarCollapsed ? 'justify-center mx-2' : ''}`}>
              {user?.profilePicture ? (
                <div 
                  className="rounded-full size-10 bg-cover bg-center border-2 border-brand-primary"
                  style={{ backgroundImage: `url("${user.profilePicture}")` }}
                />
              ) : (
                <div className="bg-gradient-to-br from-brand-primary/80 to-brand-primary rounded-full size-10 flex items-center justify-center text-sm font-bold text-white">
              ...
              <AnimatePresence>
                {!isSidebarCollapsed && (
                  ...
                  >
                    <div className="text-sm font-semibold text-slate-900 dark:text-white truncate">{user?.fullName || t("roles.parent")}</div>
        ...
        <header className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-white/10 bg-white dark:bg-[#0a192f] shadow-sm dark:shadow-none z-20 transition-colors duration-300">
          ...
          <div className="flex items-center gap-3 flex-1 max-w-xl hidden lg:flex">
            ...
            <input
              ...
              className={`h-10 w-full px-3 bg-slate-100 dark:bg-[#112240] border border-slate-200 dark:border-white/10 rounded-lg text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-primary/30 ${isRTL ? 'order-1 text-right' : 'order-2 text-left'}`}
              value={searchQuery}
          ...
          <div className="flex items-center gap-3 ms-auto">
            ...
            {/* User Avatar */}
            <Link to="/settings">
              <div className="size-10 cursor-pointer hover:ring-2 hover:ring-brand-primary rounded-full transition-all duration-200">
                {user?.profilePicture ? (
                  <div 
                  ...
                  <div className="bg-gradient-to-br from-brand-primary/80 to-brand-primary rounded-full size-10 flex items-center justify-center text-sm font-bold text-white border-2 border-slate-200 dark:border-white/20">
                    {user?.fullName?.charAt(0).toUpperCase() || user?.username?.charAt(0).toUpperCase() || 'P'}
                  </div>
                )}
              </div>
