...
export default function Pricing() {
  ...
  const plans = [
    ...
    },
    {
      ...
      link: user ? "/activate-free-trial" : "/login",
      highlight: false
    },
    {
      name: "Pro",
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* Hero */}
      <section className="bg-muted/30 pt-32 pb-16 border-b relative overflow-hidden">
          ...
          >
            ...
            <div className="flex items-center justify-center gap-4 mb-8">
              ...
              <span className={`text-sm font-medium flex items-center gap-2 ${annual ? 'text-foreground' : 'text-muted-foreground'}`}>
                Annually <span className="bg-green-100 dark:bg-green-950/50 text-green-700 dark:text-green-400 text-xs px-2 py-0.5 rounded-full font-semibold">Save 20%</span>
              </span>
            </div>
          </motion.div>
                ...
                <Card className={`w-full flex flex-col relative ${plan.highlight ? 'border-primary shadow-xl scale-105 z-10' : 'border-border shadow-sm'}`}>
                  ...
                  <CardFooter>
                    <Link href={plan.link} className="w-full">
                      <Button 
                        className={`w-full ${plan.highlight ? 'font-bold' : 'bg-muted text-foreground hover:bg-muted/80 border-none shadow-none'}`}
                      >
                        {plan.cta}
                      </Button>
                    </Link>
