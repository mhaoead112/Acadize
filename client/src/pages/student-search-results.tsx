...
import { Search, BookOpen, FileText, Calendar, MessageSquare, Award } from "lucide-react";
...
export default function StudentSearchResultsPage() {
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'courses' | 'assignments' | 'announcements' | 'events'>('all');
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      window.history.replaceState({}, '', `/student/search?q=${encodeURIComponent(searchQuery.trim())}`);
  ...
  const performSearch = async (query: string) => {
    ...
    try {
      ...
      const [coursesRes, assignmentsRes, announcementsRes] = await Promise.all([
        fetch(apiEndpoint("/api/enrollments/student"), { headers }).catch(() => null),
        fetch(apiEndpoint("/api/assignments/student"), { headers }).catch(() => null),
        fetch(apiEndpoint("/api/announcements/student"), { headers }).catch(() => null)
      ]);
      ...
      setResults({
        ...
        events: [] // Calendar events can be added later
      });
    } catch (error) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (searchQuery.trim()) {
      ...
      window.history.pushState({}, '', `/student/search?q=${encodeURIComponent(searchQuery.trim())}`);
      performSearch(searchQuery.trim());
    }
  };

  const totalResults = useMemo(() => {
    return results.courses.length + results.assignments.length + 
