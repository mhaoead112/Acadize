...
// Postgres (Neon) Configuration - Use AI_DB_* variables
const DB_HOST = process.env.AI_DB_HOST || process.env.DB_HOST || 'ep-icy-dust-ad0b7juf-pooler.c-2.us-east-1.aws.neon.tech';
const DB_PORT = parseInt(process.env.AI_DB_PORT || process.env.DB_PORT || "5432");
const DB_USER = process.env.AI_DB_USER || process.env.DB_USER || 'neondb_owner';
const DB_PASSWORD = process.env.AI_DB_PASSWORD || process.env.DB_PASSWORD || 'npg_0opIwMRy8vQP';
const DB_NAME = process.env.AI_DB_NAME || process.env.DB_NAME || 'neondb';
...
console.log(`ℹ️  Using Generation Model: ${GEN_MODEL}`);
console.log(`ℹ️  Using Database: ${DB_HOST} (${DB_NAME})`);

if (!AI_API_KEY) {
    throw new Error("AI_API_KEY missing. Put AI_API_KEY=... in .env");
}
...
const PERSONAS: Record<string, Persona> = {
    ...
    DOCTOR: {
        ...
        Tone: Professional, serious, and precise., answer is short and focuses to the pont , no alot of talking , direct to the point
...
async function produceAnswer(question: string): Promise<ResponseOutput> {
    ...
    try {
        qEmb = await retry(() => embedQuery(question));
    ...
    try {
        gen = await retry(() => generateAnswer(prompt));
    } catch (e) {
