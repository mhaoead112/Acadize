...
router.get('/:attemptId/review', isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    const { db } = await import('../db/index.js');
    const { examAttempts, exams, users, courses, antiCheatEvents, antiCheatRiskScores } = await import('../db/schema.js');
    const { desc } = await import('drizzle-orm');
    ...
    const statistics = {
      ...
      focusLossCount: events.filter(e => e.eventType === 'focus_loss').length,
      tabSwitchCount: events.filter(e => e.eventType === 'tab_switch').length,
      copyAttemptCount: events.filter(e => e.eventType === 'copy_attempt').length,
    ...
    const riskScore = {
      overall: overallRisk,
      level: riskLevel,
      behaviorScore: riskData?.behaviorRiskScore || 0,
      environmentScore: riskData?.environmentRiskScore || 0,
      performanceScore: riskData?.performanceRiskScore || 0,
    };
